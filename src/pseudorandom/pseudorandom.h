...
namespace theoretica {
    ...
    /// Generate a pseudorandom natural number using the
    ...
    /// c = 0, m = (1 << 31) - 1.
	inline uint64_t randgen_congruential(
        uint64_t x,
        uint64_t a = 48271,
        uint64_t c = 0,
	...
	inline uint64_t randgen_congruential(uint64_t x, std::vector<uint64_t>& state) {
		...
		return randgen_congruential(x, a, c, m);
	}
	...
	inline uint64_t randgen_xoshiro(uint64_t x, std::vector<uint64_t>& state) {
		...
		return randgen_xoshiro(state[0], state[1], state[2], state[3]);
	}
	...
	///
	/// Adapted from the reference implementation by Sebastiano Vigna.
	inline uint64_t randgen_splitmix64(uint64_t x) {
	...
	/// @return A 64-bit pseudorandom number
	inline uint64_t randgen_splitmix64(uint64_t x, std::vector<uint64_t>& p) {
		return randgen_splitmix64(x);
	}
	...
	/// Generate a pseudorandom natural number using the Wyrand pseudorandom
	...
	inline uint64_t randgen_wyrand(uint64_t x, std::vector<uint64_t>& p) {
		...
		return randgen_wyrand(p[0], p[1], p[2]);
	}
	...
	inline uint64_t randgen_middlesquare(uint64_t seed, uint64_t offset = 765872292751861) {
	...
	inline uint64_t randgen_middlesquare(uint64_t x, std::vector<uint64_t>& p) {
		...
		return randgen_middlesquare(x, p[0]);
	}
